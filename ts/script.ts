function calculateDates() {
  ...
  function calculateDate(tag:HTMLSpanElement|any) {
    ...
    const end: Date = tag.dataset.end
      ? new Date(tag.dataset.end)
      : new Date();
  ...
  document
    .querySelectorAll(".calculate-date")
    .forEach(calculateDate);
}
...
function initForm(){
    ...
    function error() {
      status.innerHTML = "Oops! There was a problem. Please try again or try finding me on the social links in the website footer!";
    }
